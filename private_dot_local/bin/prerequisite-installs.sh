...
prompt_yes() {
  local message="$1" timeout="${2:-$PROMPT_TIMEOUT}" reply=""
  if [[ $timeout -eq 0 ]]; then
    reply="Y"
  else
    # Ignore non-zero exit codes from read when timing out (set -e would abort otherwise)
    read -t "$timeout" -n 1 -r -p "${message}" reply || true
    echo
  fi
  [[ $reply =~ ^[Yy]$ ]]
}
...
if [[ ! $* == *"--auto-yes"* ]]; then
  if ! prompt_yes "${PURPLE}Are you happy to continue? (y/N)${RESET} "; then
